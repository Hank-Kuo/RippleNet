...
class RippleNet_replace(nn.Module):
    ...
    def forward(
        ...
        o_list, _, prob_list = self._key_addressing(h_emb_list, r_emb_list, t_emb_list, item_embeddings)
...
# With low frequency
###################################
class RippleNet_head01234_replace_multi_view(nn.Module):
    def __init__(self, args):
        super(RippleNet_head01234_replace_multi_view, self).__init__()
    ...
    def forward(
        ...
        o_list, _ = self._key_addressing(h_emb_list, r_emb_list, t_emb_list, item_embeddings)
        low_o = self._key_addressing2(h_emb_list, r_emb_list, t_emb_list, item_embeddings)
        scores = self.predict(item_embeddings, o_list, low_o)
    ...
    def _key_addressing2(self, h_emb_list, r_emb_list, t_emb_list, item_embeddings):
        total_Rh = [h_emb_list[0]]
        total_t = [h_emb_list[0]]
        ...
        for hop in range(self.n_hop):
            ...
            Rh = torch.squeeze(torch.matmul(r_emb_list[hop], h_expanded))
            total_Rh.append(Rh)
            total_t.append(t_emb_list[hop])
        # 2 -4 ? 
        # [batch_size, n_memory*n_hop, dim]
        total_Rh_tens = torch.cat((total_Rh), dim=1)
        # [batch_size, n_memory*n_hop, dim]
        total_t_tens = torch.cat((total_t), dim=1)
        # [batch_size, dim, 1]
        v = torch.unsqueeze(item_embeddings, dim=2)

        # [batch_size, n_memory*n_hop]
        probs = torch.squeeze(torch.matmul(total_Rh_tens, v))
        probs_normalized = F.softmax(probs, dim=1)
        # [batch_size, n_memory*n_hop, 1]
        probs_expanded = torch.unsqueeze(probs_normalized, dim=2)
        o = (total_t_tens * probs_expanded).sum(dim=1)
        return o
    ...
    def predict(self, item_embeddings, o_list, low_o):
        y = o_list[0] +  o_list[1] + o_list[2] + o_list[3] + o_list[4] + low_o
...
# Output
###################################
class RippleNet_head01234_replace_concat(nn.Module):
    def __init__(self, args):
        super(RippleNet_head01234_replace_concat, self).__init__()
    ...
    def _init_emb(self):
        ...
        relation_emb = nn.Embedding(self.n_relation+1, self.dim * self.dim, padding_idx=0)
        transform_matrix = nn.Linear(self.dim, self.dim, bias=False)
        output_matrix = nn.Linear(self.dim* (self.n_hop+1), self.dim)

        torch.nn.init.xavier_uniform_(entity_emb.weight)
        torch.nn.init.xavier_uniform_(relation_emb.weight)
        torch.nn.init.xavier_uniform_(transform_matrix.weight)
        torch.nn.init.xavier_uniform_(output_matrix.weight)
    ...
    def predict(self, item_embeddings, o_list):
        o_tens = torch.cat((o_list), dim=1)
        y = self.output_matrix(o_tens)
        # y = o_list[0] +  o_list[1] + o_list[2] + o_list[3] + o_list[4]
