...
def find_wikidata_id(name, keywords=[], limit=1, session=None):
    ...
    try:
        ...
        if len(category) !=0:
            j = response.json()["query"]["search"]
            for i in j:
                for item in keywords:
                    if (item in i['snippet'].lower() and isCheck== False):
                        page_id = i['pageid']
                        isCheck = True
        ...
        return "entityNotFound", "entityNotFound", str(isCheck)
    ...
    return entity_id, entity_name, isCheck
