...
def process_link(link_output, item_list):
    ...
    for idx, items in enumerate(tqdm(item_list)):
        entity_id, entity_name = wikidata.find_wikidata_id(items[1], keywords=['film', 'movie', 'director', 'direct'],limit=3)
...
def clean_graph(path, new_path):
    df = pd.read_csv(path, sep="\t")
    # df = pd.DataFrame({"A":["foo",  "bar","foo",  "bar", "bar","bar1"], "C":["foo","foo","foo","bar","bar1", "bar1"]})
    new_df = df.drop_duplicates() # 去除重複ㄉ
    new_df[new_df['head'] != new_df['tail']] # 去除 head, tail 一樣的數值ㄉ
    df.shape[0] - new_df.shape[0], new_df.shape[0]
